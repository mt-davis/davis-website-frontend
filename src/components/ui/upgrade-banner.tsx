    ...
    <div className={cn("mx-auto flex items-center justify-center", className)}>
      <AnimatePresence>
        ...
        >
          ...
          <div className="relative flex flex-col sm:flex-row h-auto sm:h-[35px] items-center gap-2 sm:gap-1 rounded-[6px] border border-[#CBE7FF] bg-[#F0F7FF] p-3 sm:pl-2.5 sm:pr-1 text-sm dark:border-[#003674] dark:bg-[#06193A]">
            <button
              className="focus-visible:shadow-focus-ring rounded-xs cursor-pointer border-none bg-transparent px-0 py-1 font-sans text-[13px] font-medium text-[#002359] underline decoration-[#CAE7FF] underline-offset-[5px] outline-none hover:text-[#005FF2] hover:decoration-[#94CCFF] focus-visible:!shadow-[#008FFF] dark:text-[#EAF5FF] dark:decoration-[#003674] dark:hover:text-[#44A7FF] dark:hover:decoration-[#00408A] w-full sm:w-auto text-center sm:text-left"
            ...
            <span className="text-[0.8125rem] text-[#005FF2] dark:text-[#44A7FF] text-center sm:text-left">
              {description}
            </span>
            {onClose && (
              <button 
                onClick={onClose}
                className="m-0 flex h-6 !w-6 shrink-0 cursor-pointer items-center justify-center rounded-[4px] border-0 bg-transparent p-0 text-[#005FF2] hover:bg-[#CAE7FF] dark:hover:bg-[#012F61] absolute top-1 right-1 sm:static"
              >
